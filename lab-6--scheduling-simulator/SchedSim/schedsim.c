...
{ 
	...
	while (total_processes_completed < n) {
       ...
       for (i = 0; i < n; ++i) {
         ...
         if (process_times[i] < min_time && completion_times[i] == -1 && plist[i].art <= total_time_lap) {
           proc_with_min_time = i;
           min_time = process_times[i];
         }
       }
       ...
       if (process_times[proc_with_min_time] == 0) {
         ...
         completion_times[proc_with_min_time] = total_time_lap + 1;
       }
       ++total_time_lap;
     }
